...
sf::Vector2u getWindowResolution(Scenes::Options& options)
{
    sf::Vector2u res;

    switch (options.graphicsLevel)
    {
    case 1: res = sf::Vector2u(640, 360); break;
    case 2: res = sf::Vector2u(960, 540); break;
    case 3: res = sf::Vector2u(1280, 720); break;
    }

    return res;
}

sf::Vector2u getFullscreenResolution(Scenes::Options& options)
{
    sf::Vector2u res;

    switch (options.graphicsLevel)
    {
    case 1: res = sf::Vector2u(options.resolutionX*0.5, options.resolutionY*0.5); break;
    case 2: res = sf::Vector2u(options.resolutionX*0.75, options.resolutionY*0.75); break;
    case 3: res = sf::Vector2u(options.resolutionX, options.resolutionY); break;
    }
...
void Scenes::fractal()
{
    ...
    const sf::Vector2u windowResolution(1280, 720);
    const sf::Vector2u windowFractalResolution = getWindowResolution(options); // quality

    const sf::Vector2u fullscreenResolution(options.resolutionX, options.resolutionY);
    const sf::Vector2u fullscreenFractalResolution = getFullscreenResolution(options);
    ...
    if(!buffer.loadFromFile("./music/"+options.songName+".wav")) return;
    SoundVisualisalizer vis(buffer);

    // Window

    sf::VideoMode mode(fullscreenResolution.x, fullscreenResolution.y);
    ...
    fractal.create(fullscreenResolution, fullscreenFractalResolution);
    fractal.GetParams().path(options.path2.c_str());
    ...
    while (window.isOpen())
    {
        ...
        if (screen_mode_flip_2)
        {
            ...
            if (fullscreen)
            {
                mode = sf::VideoMode(fullscreenResolution.x, fullscreenResolution.y);
                fractalResolution = sf::VideoMode(fullscreenFractalResolution.x, fullscreenFractalResolution.y);
            }
            else
            {
                mode = sf::VideoMode(windowResolution.x, windowResolution.y);
                fractalResolution = sf::VideoMode(windowFractalResolution.x, windowFractalResolution.y);
            }
